...
export default function App() {
  ...
  const handleSubmit = async (event) => {
    ...
    //this ensures that size submission is not blank
    if (size === "") {
      setError("You need click on a size for the item")

      //if no size is submitted will stop from being submitted
      return;
    }
  ...
  return (
    <div>
      <section>
        <h1>Stock Taking App</h1>
        <p>Please click a Item type from the drop down menu, then pick at least one colour and a size. All options need an entry or it wont save to
          to the database</p>
      ...
      <section>
        ...
        <table border="1">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr>
                <td colSpan="5">No inventory has been saved yet.</td>
              </tr>
            ) : (
              items.map((item) => (
                <tr key={item.id}>
